...
- Skip rules: env `PROMPT_MAX_OFF=1`; env `PROMPT_MAX_CHILD=1` (recursion guard); prompt starts with `/`; fewer than `minWords` words (default 8); prompt starts with `!raw ` (explicit bypass); refiner returns `PASS`.
